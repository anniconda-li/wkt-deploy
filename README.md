...
| 服务 | 设备地址 | 镜像 |
| --- | --- | --- |
| AI | `http://139.129.17.67:18080` | `ghcr.io/anniconda-li/wkt-ai-server:0.3.1` |
...
AI 0.3.1 在同一个 `18080 -> 8000` TCP 映射上同时提供既有 HTTP API 和 WAI1 WebSocket。设备地址为：
...
AI 0.3.1 保留 `/ai/start`、`/ai/upload`、`/ai/finish`、`/ai/result_info`、`/ai/result_chunk`、`/ai/cancel`、`/ai/stop_audio`、`/camera/upload`、`/camera/upload/chunk`、`/camera/upload/finish`、`/camera/upload/cancel` 和 `/ai/ws?device=...&protocol=wai1`。本版本约束自我介绍只使用“我是博物馆AI讲解员。”，并限制文物介绍只出现省级及以上地名。对讲协议、OTA 协议和设备 ID 语义不变。
