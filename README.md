...
| 服务 | 设备地址 | 镜像 |
| --- | --- | --- |
| AI | `http://139.129.17.67:18080` | `ghcr.io/anniconda-li/wkt-ai-server:0.3.2` |
...
AI 0.3.2 在同一个 `18080 -> 8000` TCP 映射上同时提供既有 HTTP API 和 WAI1 WebSocket。设备地址为：

```text
ws://139.129.17.67:18080/ai/ws?device=walkie-01&protocol=wai1
```

Docker Compose 的 TCP 端口映射天然支持 WebSocket，不需要新端口、Nginx、Traefik 或额外 Upgrade 配置。
...
| 服务 | 宿主机目录 | 容器目录 | 内容 |
| --- | --- | --- | --- |
| AI | `${AI_DATA_ROOT}/uploads` | `/app/uploads` | 相机 JPEG/AOP1、相机和 WAI1 SQLite、分片临时文件、ROP1 回复 |
...
AI 0.3.2 保留 `/ai/start`、`/ai/upload`、`/ai/finish`、`/ai/result_info`、`/ai/result_chunk`、`/ai/cancel`、`/ai/stop_audio`、`/camera/upload`、`/camera/upload/chunk`、`/camera/upload/finish`、`/camera/upload/cancel` 和 `/ai/ws?device=...&protocol=wai1`。本版本撤销 0.3.1 的身份及地名输出限制，恢复 0.3.0 的地方文化讲解行为。对讲协议、OTA 协议和设备 ID 语义不变。
